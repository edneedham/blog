...
*This article assumes that the reader has read 
[The Rust Programming Language Book](https://doc.Rust-lang.org/book/) 
and has good familiarity with the language.* 
...
a node can have *two* children so that means *two* mutable references to the 
...
For us this means that the `type Item` is referring to the value or *Item* we 
...
Below is a test for the inorder traversal of our tree. As mentioned in [*Balance Without Rebalancing*](#balance), 
...
***All further testing is omitted from the rest of this article and can be 
found in the repo on my [Github](https://github.com/edneedham/iterators).***
...
the `stack` first, we ensure that the right child is popped last, *after* 
...
`IterMut`, however, *cannot* return the key, as any mutation on the key would 
